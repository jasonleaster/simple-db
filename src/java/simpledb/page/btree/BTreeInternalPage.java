...
public class BTreeInternalPage extends BTreePage {
    ...
    private final Field keys[]; // 分支点
    private final int children[]; // 子页面的指针
     ...
     * leaf or internal).
     * Specifically, the number of entries is equal to: <predicteOper>
     * floor((BufferPool.getPageSize()*8 - extra bytes*8) / (entry size * 8 + 1))
     * <predicteOper> where entry size is the size of entries in this index node
    ...
    public int getMaxEntries() {
        ...
        // key本身占用的空间大小 + children[i]指针占用的空间 + 该key对应在header中占用的空间大小
        ...
        // 1 bit for extra header
        // 父节点指针 + keys第0个元素即哑结点指针 + 1byte对应childCategory + 1bit额外的空间(这我就不明白了)
    ...
    public byte[] getPageData() {
        ...
        int zeroLen = BufferPool.getPageSize() -
                (INDEX_SIZE + CATEGORY_SIZE + header.length +
                        td.getFieldType(keyField).getLen() * (keys.length - 1)
                        + INDEX_SIZE * children.length);
     ...
     * (note that this iterator shouldn'tableId return entries in empty slots!)
     */
     ...
     */
    private class BTreeInternalPageIterator implements Iterator<BTreeEntry> {
        int curEntry = 1;
     ...
     */
    private class BTreeInternalPageReverseIterator implements Iterator<BTreeEntry> {
        int curEntry;
