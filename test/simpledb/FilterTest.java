...
public class FilterTest extends SimpleDbTestBase {
    ...
    public void getTupleDesc() {
        Predicate predicate = new Predicate(0, Predicate.Op.EQUALS, TestUtil.getField(0));
        Filter op = new Filter(predicate, scan);
    ...
    public void filterAllLessThan() throws Exception {
        Predicate predicate;
        predicate = new Predicate(0, Predicate.Op.LESS_THAN, TestUtil.getField(-5));
        Filter op = new Filter(predicate, scan);
        op.open();
        assertTrue(TestUtil.checkExhausted(op));
        op.close();
    ...
    public void filterEqualNoTuples() throws Exception {
        Predicate predicate;
        predicate = new Predicate(0, Predicate.Op.EQUALS, TestUtil.getField(5));
        Filter op = new Filter(predicate, scan);
        op.open();
